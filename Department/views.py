...
from Department.models import Department,Role,Users,Task,Task_Assignment,Review,Leave,LeaveQuota
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            send_mail(
                'Password Reset OTP',
                f'Your OTP is {otp}',
                'krishnavispute543@gmail.com',  # Replace with your email
                [email],
                fail_silently=False,
            )
...
def view_tasks(request):
    # Get the status filter from the GET parameters
    ...
    # Render the view with the tasks and statistics
...
def add_task(request):
    ...
    # Fetch employees (all or those reporting to the user)
    employees = Users.objects.filter(reporting_manager=request.user) if Users.objects.filter(reporting_manager=request.user).exists() else Users.objects.all()

    return render(request, 'add_task.html', {'employees': employees})

# Edit a task
def edit_task(request, task_id):
    task = get_object_or_404(Task, task_id=task_id)  # Ensure you're using the correct field for lookup
    if request.method == 'POST':
        # Update task fields from the form submission
        ...
        # Get the assigned_to user ID from the form and update the field
        assigned_to_id = request.POST.get('assigned_to')
        if assigned_to_id:
            task.assigned_to = Users.objects.get(id=assigned_to_id)  # Make sure the user exists in the database

        task.save()  # Save the task with the updated fields
        return redirect('viewtasks')  # Redirect after updating

    # Fetch employees based on the current user's reporting manager
    employees = Users.objects.filter(reporting_manager=request.user) if Users.objects.filter(reporting_manager=request.user).exists() else Users.objects.all()
